...
public class KamiActiveModulesUI extends AbstractComponentUI<ActiveModules>
{
    ...
    public KamiActiveModulesUI() {
        this.renderer = new CFontRenderer(new Font("Arial", 0, 18), true, false);
    }
    ...
    public void renderComponent(final ActiveModules component, final FontRenderer f) {
        ...
        final List<Module> mods = ModuleManager.getModules().stream().filter(Module::isEnabled).filter(Module::isOnArray).sorted(Comparator.comparing(module -> {
            ...
            return Integer.valueOf(renderer.getStringWidth(sb.append(string).toString()) * (component.sort_up ? -1 : 1));
        ...
        for (int j = 0; j < mods.size(); ++j) {
            ...
            final int textWidth = this.renderer.getStringWidth(text);
            final int textHeight = this.renderer.getHeight() + 1;
            final int red = rgb >> 16 & 0xFF;
            final int green = rgb >> 8 & 0xFF;
            final int blue = rgb & 0xFF;
            this.renderer.drawStringWithShadow(text, xFunc.apply(textWidth), y[0], rgb);
